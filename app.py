...
# ПРЯМАЯ ССЫЛКА (Исправил кавычку)
...
if st.session_state.app_state == "welcome":
    ...
    with tab_reg:
        new_n = st.text_input("Имя нового парня", key="reg_name_input")
        new_b = st.text_area("О нем", key="reg_bio_input")
        if st.button("СОЗДАТЬ ПРОФИЛЬ", key="reg_submit_btn"):
    ...
    if settings_sheet:
        ...
        sel_h = st.selectbox("С кем общаемся?", h_names, key="hero_dropdown")
        if st.button("УСТАНОВИТЬ СОЕДИНЕНИЕ"):
            h = next(i for i in h_data if i["Name"] == sel_h)
            st.session_state.persona = f"Ты {h['Name']}. {h['Prompt']}. Собеседник: {st.session_state.u_name}. ОБЯЗАТЕЛЬНО используй много эмодзи! ✨⚡️"
            st.session_state.current_name = h['Name']
            st.session_state.app_state = "chat"
            st.rerun()

elif st.session_state.app_state == "chat":
    # Компактный хедер: Фото слева, имя справа
    st.markdown(f"""
        <div class="chat-header">
            <img src="{USER_PHOTO}" style="width: 50px; height: 50px; border-radius: 50%; border: 2px solid #ff4b4b; object-fit: cover;">
            <div style="text-align: left;">
                <div style="color: #ff4b4b; font-size: 16px; font-weight: 600; letter-spacing: 1px;">{st.session_state.current_name.upper()}</div>
                <div style="color: #00ff00; font-size: 10px; opacity: 0.8;">● В СЕТИ</div>
            </div>
        </div>
    """, unsafe_allow_html=True)
